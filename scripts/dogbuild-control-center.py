...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; color:var(--ink); background:var(--bg); font:16px/1.45 ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif; }
    ...
    #projects { display:grid; grid-template-columns:repeat(auto-fit,minmax(300px,1fr)); gap:18px; } .handoff-help { margin:-8px 0 20px; padding:12px 14px; border:1px solid #f1c5ba; border-radius:12px; background:#fff7f4; color:#7d372a; font-size:14px; } .handoff-help code { color:var(--ink); font:12px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace; } .card { position:relative; overflow:hidden; border:1px solid var(--line); border-radius:18px; background:var(--paper); box-shadow:0 8px 26px rgba(22,32,51,.06); padding:22px; } .card::before { content:""; position:absolute; inset:0 auto 0 0; width:5px; background:var(--accent); } .card.attention::before { background:#e46b52; } .title { display:flex; justify-content:space-between; gap:12px; align-items:start; } h2 { margin:0; font-size:22px; letter-spacing:-.02em; } .updated { color:var(--muted); font-size:12px; white-space:nowrap; } .git { margin:6px 0 20px; color:var(--muted); font:12px/1.4 ui-monospace,SFMono-Regular,Menlo,monospace; }
...
<body>
  ...
  <main>
    ...
    <p id="handoff-help" class="handoff-help" hidden></p>
  ...
  <script>
    ...
    async function refresh() {
      try {
        ...
        const handoffHelp = document.getElementById('handoff-help');
        handoff.hidden = !data.handoff_status;
        if (data.handoff_status) handoff.textContent = `Shared handoff: ${data.handoff_status}`;
        handoffHelp.hidden = !['missing', 'stale'].includes(data.handoff_status);
        if (!handoffHelp.hidden) handoffHelp.innerHTML = 'Shared handoff needs an update. In this ai-setups folder, run <code>python3 scripts/dogbuild-sync-project-updates.py</code>.';
